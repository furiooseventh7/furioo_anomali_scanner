"""
Whale Sonar — mendeteksi akumulasi smart money, distribusi,
aggressive buy/sell, dan bid wall besar.
"""
import pandas as pd
import numpy as np
import logging
...
class WhaleSonarResult:
    buy_pressure       : float = 0.5   # 0–1 (1 = all buy)
    sell_pressure      : float = 0.5
    avg_whale_trade_usd: float = 0.0
    ob_imbalance       : float = 0.0   # positif = bid dominan
    bid_ask_ratio      : float = 1.0
    bid_wall_usd       : float = 0.0
    is_accumulating    : bool  = False
    is_distributing    : bool  = False
    aggressive_buy_pct : float = 0.0   # % volume dari aggressive buy
    taker_buy_ratio    : float = 0.5   # dari kline taker_buy_base
    signals            : list  = field(default_factory=list)
    score              : float = 0.0   # kontribusi ke total score (0–25)

def analyze_whale(symbol: str) -> WhaleSonarResult:
    res = WhaleSonarResult()

    # ── 1. Aggregate Trades Analysis ─────────────────────
    df_trades = get_agg_trades(symbol, limit=500)
    if not df_trades.empty:
        # Top 10% transaksi by value = whale
        ...
        if not whale_df.empty:
            ...
            # Aggressive buy % dari total volume
    ...
    # ── 2. Kline Taker Buy Ratio (lebih akurat) ───────────
    ...
    if not df_kline.empty:
        ...
        # Cek 3 jam terakhir vs 21 jam sebelumnya
    ...
    if res.taker_buy_ratio >= 0.65:
        res.signals.append(f"📈 Taker buy ratio tinggi {res.taker_buy_ratio*100:.0f}% (24H kline)")
        score += 5
