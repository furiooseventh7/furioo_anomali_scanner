...
def format_and_send_signal(sig, vr_llama=None, entry_timing=None, onchain_res=None, vol_spike_res=None) -> bool:
    """
    ...
    msg  = f"{le} <b>CMI-ASS v7 SIGNAL</b> {le}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    # ── On-Chain Whale Activity (v7 NEW) ──────────────────
    if onchain_res and onchain_res.found_onchain:
        try:
            from engine.onchain_tracker import format_onchain_section
            msg += format_onchain_section(onchain_res)
        except Exception as e:
            logger.debug(f"OnChain format error: {e}")

    # ── Volume Spike (v7 NEW) ─────────────────────────────
    if vol_spike_res and vol_spike_res.has_spike:
        try:
            from engine.volume_spike_detector import format_volume_spike_section
            msg += format_volume_spike_section(vol_spike_res)
        except Exception as e:
            logger.debug(f"VolSpike format error: {e}")
    ...
    vs_score = getattr(sig, "vol_spike_score", 0.0)
    oc_score = getattr(sig, "onchain_score", 0.0)
    msg += f"📋 <b>Score Breakdown:</b>\n"
    msg += f"  🐋 Whale CEX:     {sig.whale_score:.0f}/25\n"
    ...
    if vs_score > 0:
        msg += f"  📊 Vol Spike:     {vs_score:.0f}/25\n"
    if oc_score > 0:
        msg += f"  ⛓️ On-Chain DEX:  {oc_score:.0f}/30\n"
    ...
    msg += f"⚡ <i>CMI-ASS v7 | OnChain+VolSpike+DefiLlama+SMC+Whale | by Furiooseventh</i>\n"
...
def send_startup_message() -> bool:
    msg  = "🚀 <b>CMI-ASS v7 Scanner AKTIF</b>\n"
    msg += "━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += "🐋 Whale Sonar CEX: ON\n"
    ...
    msg += "💚 DefiLlama Fundamental Engine: ON\n"
    msg += "  ↳ P/S Ratio · P/F Ratio · TVL/MCap · Revenue Growth\n"
    msg += "🎯 Precision Entry Engine: ON\n"
    msg += "  ↳ Wyckoff Phase · MARKUP_IMMINENT · ACCUMULATING\n"
    msg += "🖼️ TradingView Chart Screenshot: ON\n"
    msg += "⛓️ <b>On-Chain DEX Whale Tracker: ON ← NEW v7</b>\n"
    msg += "  ↳ Flipside SQL · Dune Analytics · DexScreener\n"
    msg += "  ↳ Query: transaksi DEX > $100K dalam 4H terakhir\n"
    msg += "  ↳ Wallet baru + volume masif = STEALTH ACCUMULATION alert\n"
    msg += "📊 <b>CEX Volume Spike Detector: ON ← NEW v7</b>\n"
    msg += "  ↳ Bitget + MEXC multi-TF (1H/4H/1D) baseline 30D\n"
    msg += "  ↳ Klasifikasi: BREAKOUT_BUY · ACCUMULATION · STOP_HUNT · DUMP\n"
    msg += "  ↳ Standalone alert untuk spike CRITICAL sebelum scan penuh\n"
