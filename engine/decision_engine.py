...
def make_decision(
    ...
    fear_greed    : dict,
    ta_res        = None,
    narr_res      = None,   # ← NarrativeResult (optional)
) -> FinalSignal:
    """
    ...
    # BUY SPOT: tidak ada di futures atau micro cap
        ...
        # Longgarkan: BUY SPOT jika TA bullish ATAU score cukup tinggi
        if ta_bullish or ta_gated or score >= 40:
            signal_type = "BUY SPOT"
        else:
            signal_type = "WATCH"

    # LONG: ada di futures, TA bullish (gated lebih diutamakan tapi tidak wajib)
    elif score >= 40 and whale_res.buy_pressure >= 0.50 and ta_bullish:
        signal_type = "LONG"

    # LONG: score sangat tinggi meski TA tidak bullish
