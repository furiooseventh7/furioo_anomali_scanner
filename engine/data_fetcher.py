...
# Suppress numpy RuntimeWarning/FutureWarning — handled explicitly in code
warnings.filterwarnings("ignore", category=RuntimeWarning)
warnings.filterwarnings("ignore", category=FutureWarning)

# Dead-endpoint cache: path prefixes that returned 404/403 are skipped on
# subsequent calls, saving ~5s timeout per coin per failing endpoint.
...
def _get_bitget_futures(path: str, params: dict = None) -> Optional[dict | list]:
    ...
    Endpoint yang sudah terbukti 404 langsung di-skip (dead-endpoint cache).
    """
    dead_key = (BITGET_FUTURES_BASE, path)
    if dead_key in _DEAD_ENDPOINTS:
        return None
    ...
    try:
        ...
        if r.status_code == 404:
            logger.warning(f"Bitget Futures GET {path}: 404 — endpoint di-blacklist")
            _DEAD_ENDPOINTS.add(dead_key)
            return None
        r.raise_for_status()
        body = r.json()
        if body.get("code") == "00000":
...
def _get_mexc_futures(path: str, params: dict = None) -> Optional[dict | list]:
    ...
    Endpoint yang sudah terbukti 404/403 langsung di-skip (dead-endpoint cache)
    untuk menghindari timeout berulang yang memperlambat scan.
    """
    # Gunakan path prefix (/api/v1/contract/kline/openInterest, /api/v1/contract/long_short)
    # sebagai dead-endpoint key — bukan full path termasuk symbol.
    path_prefix = "/".join(path.split("/")[:5])  # e.g. /api/v1/contract/long_short
    dead_key = (MEXC_FUTURES_BASE, path_prefix)
    if dead_key in _DEAD_ENDPOINTS:
        return None
    ...
    try:
        ...
        if r.status_code in (404, 403):
            logger.warning(f"MEXC Futures GET {path}: {r.status_code} — endpoint di-blacklist")
            _DEAD_ENDPOINTS.add(dead_key)
            return None
        r.raise_for_status()
        body = r.json()
    ...
    if data and isinstance(data, list):
        ...
        for t in data:
            ...
            rows.append({
                ...
                "qty"             : qty,
                "value"           : price * qty,
                "time"            : pd.to_datetime(int(t.get("ts", 0) or 0), unit="ms"),
