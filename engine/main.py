...
def scan_single(row: pd.Series, futures_symbols: set,
    ...
    if price <= 0 or vol_24h < MIN_VOLUME_24H_USD:
        return None
    if chg_24h > 80:    # sudah pump besar → skip
        return None

    try:
        # ── Analisis paralel (sequential karena rate limit) ─
...
def run():
    ...
    # ── Filter & prioritisasi kandidat ───────────────────
    candidates = df_ticker[
        (df_ticker["quoteVolume"] >= MIN_VOLUME_24H_USD) &
        (df_ticker["priceChangePercent"].abs() < 80)
    ].copy()

    # Prioritaskan: volume spike (volume tinggi tapi harga belum pump besar)
    ...
    for _, row in candidates.iterrows():
        ...
        time.sleep(0.1)   # global rate limit buffer

    # ── Sort by score & kirim alert ───────────────────────
    ...
    for sig in signals:
        if should_alert(sig.alert_level, ALERT_MIN_LEVEL):
            ...
            if success:
                alerts_sent += 1
            time.sleep(1.5)   # jeda antar pesan Telegram
