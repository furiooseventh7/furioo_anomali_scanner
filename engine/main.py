...
def scan_single(row: pd.Series, futures_symbols: set,
    ...
    try:
        ...
        try:
            ...
            # BTC return sebagai market proxy untuk dual momentum
            btc_chg = float(row.get("btcChange24h", chg_24h * 0.5))
            quant_res = analyze_quant(symbol, price, df_1h, df_4h, btc_chg)
        except Exception as qe:
            logger.debug(f"quant_engine skip {symbol}: {qe}")
            quant_res = None
        ...
        # Simpan quant result ke extra_context
        if quant_res is not None:
            signal.extra_context["quant"] = quant_res
