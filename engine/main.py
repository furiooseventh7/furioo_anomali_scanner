    ...
    try:
        ...
        if signal.confidence_score >= MIN_CONFLUENCE_SCORE and signal.signal_type != "NEUTRAL":
            logger.info(
                ...
                f"TA: {signal.ta_bias} | MTF: {getattr(ta_res,'mtf_bias','N/A')}"
            )
            return signal
        else:
            logger.debug(
                ...
                f"MTF:{getattr(ta_res,'mtf_bias','N/A')} | gated:{getattr(ta_res,'is_gated',False)}"
            )
            return signal
...
def run():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures_map = {
            executor.submit(scan_single, row, futures_set, cg_data, fear_greed): row
            for row in rows
        }
